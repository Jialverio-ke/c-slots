controller.A.onEvent(ControllerButtonEvent.Pressed, function () {
    let Number_we_pick: number[] = []
    for (let index = 0; index < 3; index++) {
        TO_Display = randint(0, 4)
        game.showLongText(Fruits[TO_Display], DialogLayout.Bottom)
        Number_we_pick.unshift(TO_Display)
    }
    game.showLongText(Number_we_pick, DialogLayout.Center)
})
let TO_Display = 0
let Fruits: string[] = []
